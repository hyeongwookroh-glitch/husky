...
export async function createChannel(config) {
  ...
  const MEMORY_BASE = process.env.HUSKY_MEMORY_DIR
    ? process.env.HUSKY_MEMORY_DIR.replace(/^~/, os.homedir())
    : path.join(os.homedir(), 'Documents', 'Husky_Memory')
  ...
  setInterval(() => {
    mcp.notification({
      method: 'notifications/claude/channel',
      params: {
        content: `[SYSTEM] 30 minutes elapsed. Write session notes to ~/Documents/Husky_Memory/session_notes/husky/ (today's date file). Append if exists.`,
